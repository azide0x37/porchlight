...
class PorchlightHTTPServer(ThreadingHTTPServer):
    ...
    def setup_payload(self) -> dict[str, object]:
        return {
            ...
            "openai": self.openai_payload(),
            "wifi": self.wifi_payload(),
        }

    def openai_payload(self, settings: dict[str, object] | None = None) -> dict[str, object]:
        payload = dict(settings or masked_openai_settings(self.config.config_dir))
        payload["analysis_status"] = self.analysis_status()
        return payload

    def analysis_status(self) -> dict[str, object]:
        path = self.config.www_dir / "analysis.json"
        if not path.is_file():
