...
class MusterContractTest(unittest.TestCase):
    def test_systemd_units_call_current_release_bin(self):
        ...
        ai_service = (ROOT / "systemd/porchlight-ai-analysis.service").read_text(encoding="utf-8")
        ai_timer = (ROOT / "systemd/porchlight-ai-analysis.timer").read_text(encoding="utf-8")
        ...
        for name in [
            ...
            "porchlight-ai-analysis.service",
            "porchlight-ai-analysis.timer",
    ...
    def test_installer_and_doctor_cover_mqtt_adapter_dependency(self):
        ...
        self.assertIn("install_static_webroot", install)
        self.assertIn('"/var/lib/$PROJECT/www"', install)
        update = (ROOT / "bin/update.sh").read_text(encoding="utf-8")
        self.assertIn("update failed during install; rolled back", update)
        ...
        self.assertIn("ai analysis status rendered", doctor)
    ...
    def test_dashboard_assets_are_packaged_and_runtime_json_backed(self):
        ...
        self.assertIn("/style.css?v=2.3.1", index)
        self.assertIn("/app.js?v=2.3.1", index)
        self.assertIn("Porchlight v2.3.1", index)
        ...
        self.assertIn("/analysis.json", app)
        self.assertIn("/api/setup/status", app)
        self.assertIn("renderSettings", app)
        self.assertIn("Save MQTT", app)
        self.assertIn("Save AI Settings", app)
        self.assertIn("Last analysis", app)
        self.assertIn("renderIrregularitiesSection", app)
