...
OPENAI_KEYS = (
    "OPENAI_API_KEY",
    "PORCHLIGHT_AI_ANALYSIS_ENABLE",
    "PORCHLIGHT_AI_MODEL",
    "PORCHLIGHT_AI_SERVICE_TIER",
)
...
DEFAULT_OPENAI = {
    "OPENAI_API_KEY": "",
    "PORCHLIGHT_AI_ANALYSIS_ENABLE": "0",
...
AI_SERVICE_TIERS = {"auto", "default", "flex", "priority"}
...
def masked_openai_settings(config_dir: Path) -> dict[str, object]:
    ...
    return {
        ...
        "analysis_enabled": bool_value(values.get("PORCHLIGHT_AI_ANALYSIS_ENABLE"), False),
        "model": values.get("PORCHLIGHT_AI_MODEL") or DEFAULT_OPENAI["PORCHLIGHT_AI_MODEL"],
        "service_tier": values.get("PORCHLIGHT_AI_SERVICE_TIER") or DEFAULT_OPENAI["PORCHLIGHT_AI_SERVICE_TIER"],
    }
...
def update_openai_settings(config_dir: Path, payload: dict[str, object]) -> dict[str, object]:
    ...
    if "analysis_enabled" in payload:
        updates["PORCHLIGHT_AI_ANALYSIS_ENABLE"] = "1" if bool(payload["analysis_enabled"]) else "0"
    if "model" in payload:
        updates["PORCHLIGHT_AI_MODEL"] = validate_openai_model(payload["model"])
    if "service_tier" in payload:
        updates["PORCHLIGHT_AI_SERVICE_TIER"] = validate_openai_service_tier(payload["service_tier"])
...
def validate_openai_model(value: object) -> str:
    text = str(value or "").strip()
    if not text:
        return DEFAULT_OPENAI["PORCHLIGHT_AI_MODEL"]
    if len(text) > 128 or not MODEL_RE.match(text):
        raise ValueError("OpenAI model must be a model id without spaces")
    return text


def validate_openai_service_tier(value: object) -> str:
    text = str(value or "").strip().lower()
    if not text:
        return DEFAULT_OPENAI["PORCHLIGHT_AI_SERVICE_TIER"]
    if text not in AI_SERVICE_TIERS:
        raise ValueError("OpenAI service tier must be auto, default, flex, or priority")
    return text
