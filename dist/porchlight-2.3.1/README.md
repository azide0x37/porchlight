...
service also exposes constrained `/api/setup/*` JSON endpoints for MQTT,
OpenAI key, and appliance setup writes under `/etc/porchlight`; it does not
accept arbitrary commands or service names from the browser. The dashboard reads
`/status.json`, `/hosts.json`, `/services.json`,
`/changes.json`, `/analysis.json`, and `/snapshot.json` at runtime, which keeps
the appliance UI in sync with the latest scanner render.
...
AI-analysis settings without requiring SSH. Finishing setup writes
...
| --- | --- | --- |
| systemd owns lifecycle | PASS | `systemd/porchlight-ha-mqtt-bridge.service` and `systemd/porchlight-ai-analysis.service` call `/opt/porchlight/current/bin/...` |
| systemd timer owns scheduled refresh | PASS | `systemd/porchlight-scan.timer`, `systemd/porchlight-ha-mqtt-bridge.timer`, and `systemd/porchlight-ai-analysis.timer` run scan, publish, and sidecar loops |
...
| units call `/opt/porchlight/current/bin/...` | PASS | `systemd/porchlight-ha-mqtt-bridge.service`, `systemd/porchlight-ai-analysis.service` `ExecStart` |
