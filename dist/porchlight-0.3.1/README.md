...
- `nmap -sT -n --open` for allowed active scans

This is intentionally conservative. By default Porchlight refuses active scans
outside private IPv4 space and refuses networks broader than
`PORCHLIGHT_MAX_PREFIX=/24`. On a wider LAN it still records neighbor-table,
ARP, and Tailscale evidence, then marks active scanning as degraded/blocked.
