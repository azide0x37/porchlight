...
Active scanning is private-network only by default and refuses CIDRs broader
than `PORCHLIGHT_MAX_PREFIX`, which defaults to `/24`. Porchlight still records
passive neighbor-table, ARP, and Tailscale evidence when active scanning is
blocked. UDP scans, NSE scripts, full TCP scans, and Tailscale active scans are
disabled by default.
