...
<body>
  ...
  <main>
    <section>
      <h2>Hosts</h2>
      <table id="hosts"><thead><tr><th>Name</th><th>IP</th><th>MAC</th><th>Status</th><th>Interface</th></tr></thead><tbody></tbody></table>
    </section>
    <section>
      <h2>Services</h2>
      <table id="services"><thead><tr><th>Host</th><th>IP</th><th>Port</th><th>Service</th><th>Product</th></tr></thead><tbody></tbody></table>
    </section>
  </main>
...
async function main() {
  ...
  document.querySelector("#summary").textContent =
    `${status.hosts_seen || 0} hosts, ${status.active_hosts || 0} active, ${status.open_ports || 0} open ports`;
  const hostBody = document.querySelector("#hosts tbody");
  for (const host of hosts.hosts || []) {
    hostBody.appendChild(row([host.display_name || host.hostname || "", host.ip, host.mac, host.status || (host.active ? "active" : "inactive"), host.interface]));
  }
  const serviceBody = document.querySelector("#services tbody");
  for (const service of services.services || []) {
    serviceBody.appendChild(row([service.display_name, service.ip, `${service.proto}/${service.port}`, service.service_name, service.product]));
  }
}
main().catch((error) => {
