...
- Added opt-in AI-generated environment, protocol, host, and irregularity
  summaries backed by an asynchronous `porchlight-ai-analysis` systemd sidecar.
- Added configurable AI analysis enablement, model id, and service tier settings
  while continuing to mask the stored OpenAI API key.
- Added scan-to-scan irregularity tracking for new or missing hosts, opened or
  closed services, and service metadata changes, with discover-mode refreshes
  preserved as non-closing observations.
