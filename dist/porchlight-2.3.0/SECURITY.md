...
AI analysis is disabled by default. When enabled, Porchlight sends a bounded
rendered network snapshot and recent scan irregularities to the configured
OpenAI model through `porchlight-ai-analysis.service`. Generated summaries are
advisory only and are never used as command input. If the key is missing, the
API call fails, or the response shape is invalid, the dashboard falls back to
local deterministic analysis.

## Setup API

`porchlight-web.service` exposes `/api/setup/*` for local appliance setup. The
API validates MQTT host, port, topic, node fields, OpenAI key shape, model id,
and service tier before writing env files. Wi-Fi writes are accepted only when
appliance mode is enabled in `/etc/porchlight/setup.env`. Setup side effects
beyond file writes go through `/run/porchlight/setup-action`, which is consumed
by an allowlisted systemd helper.
